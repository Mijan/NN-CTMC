...
class MLEstimator:
    ...
    def plotMLEstimates(self, mle_transition_matrix: np.array):
        ...
        fig = self.__createPropensityPlot(estimated_propensities, true_propensities)
        return fig

    def __createPropensityPlot(self, estimated_propensities, true_propensities):
        nrows, ncols = getRowsCols(self.__num_reactions)
        # Visualization
        fig, axs = plt.subplots(nrows, ncols, figsize=(5*ncols, 5*nrows))
        for rct_nbr in range(self.__num_reactions):
            if nrows >1:
            ...
            for k, s in enumerate(self.__stoich):
                if k != rct_nbr and np.array_equal(s, self.__stoich[rct_nbr]):
